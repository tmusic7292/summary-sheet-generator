...
#!/usr/bin/env python3
"""
Summary Sheet Tool v15.0 — FIXED PDF Extraction
Features:
- Correct extraction from PDF text structure
- NOx+HC from Emission Limits (section before Concentrations)
- S_Nox, THC, S_THC from Specific Results section
- CO, PM, PN properly extracted
...
# --- CONFIG ---
APP_TITLE = "Summary Sheet Tool v15.0 — FIXED Edition"
ADDITIONAL_HEADERS = [
...
def parse_pdf_fixed(path: str):
    """
    Extract from actual PDF structure:
    
    1. Emission Limits section → NOx+HC, CO uncorrected, PM, PN
    2. Specific Results section → S_Nox (NOX), THC, S_THC (same as THC)
    3. Mass Results section → Can extract CO if needed
    """
    nox_hc = ""
    co = ""
    pm = ""
    ...
    try:
        with pdfplumber.open(path) as pdf:
            for page in pdf.pages:
                ...
                # ===== EMISSION LIMITS (before Concentrations) =====
                if "Emission limits - Stage V NRE" in txt:
                    lines = txt.splitlines()
                    for line in lines:
                        # Extract NOX+HC
                        if "NOX+HC" in line or "NOx+HC" in line:
                            # Format: "NOX+HC [g/kWh]  3.267  4.700  Passed..."
                            parts = line.split()
                ...
                if "Specific results" in txt:
                    ...
                    for i, line in enumerate(lines):
                        ...
                        if in_specific:
                            # Stop when hitting next section
                            if any(x in line for x in ["Fuel consumption", "Engine performance", "Mass results"]):
                                break
                            
                            # Extract S_Nox from "NOX [g/kWh]  3.244"
                            if re.search(r'^\s*NOX\s*\[g/kWh\]', line, re.IGNORECASE) or re.search(r'^\s*NOx\s*\[g/kWh\]', line, re.IGNORECASE):
                                parts = line.split()
                                # Look for the numeric value after [g/kWh]
                                for j in range(len(parts)):
                                    if "[g/kWh]" in parts[j] and j + 1 < len(parts):
                                        val = parts[j + 1]
                                        if re.match(r'^[\d.]+$', val):
                                            s_nox = val
                                            break
                            
                            # Extract THC from "THC [g/kWh]  0.023"
                            if re.search(r'^\s*THC\s*\[g/kWh\]', line, re.IGNORECASE):
                                ...
                                for j in range(len(parts)):
                                    if "[g/kWh]" in parts[j] and j + 1 < len(parts):
                                        ...
                                        if re.match(r'^[\d.]+$', val):
                                            thc = val
                                            s_thc = val  # S_THC is same as THC
                                            break
    
    except Exception as e:
        return nox_hc, s_nox, thc, s_thc, co, pm, pn, f"Error: {e}"
...
class App(tk.Tk):
    ...
    def _build_ui(self):
        ...
        # ===== TITLE =====
        title_label = ttk.Label(main_frame, text="📊 Summary Sheet Tool v15.0", style='Title.TLabel')
    ...
    def _process_file_to_row(self, f):
        ...
        if low.endswith(".pdf"):
            nox_hc, s_nox, thc, s_thc, co, pm, pn, _remark = parse_pdf_fixed(f)
