...
#!/usr/bin/env python3
"""
Summary Sheet Tool v16.0 — SIMPLIFIED & FAST PDF Extraction
Features:
- Search for "Tailpipe (EBH02) bench results" first
- Extract Specific Results section data only
- Fast regex-based extraction
- NOx+HC, S_Nox, THC, S_THC, CO, PM, PN
"""
...
# --- CONFIG ---
APP_TITLE = "Summary Sheet Tool v16.0 — FIXED Edition"
ADDITIONAL_HEADERS = [
...
# ========== FAST & SIMPLE PDF PARSING ==========
def extract_value_from_line(line: str):
    """Extract numeric value from a line like 'NOX [g/kWh] 2.830'"""
    # Remove everything up to [g/kWh] or [ppm], then get the first number
    match = re.search(r'\[\s*g/kWh\s*\]\s*([\d.eE+-]+)', line)
    if match:
        return match.group(1)
    return ""

def parse_pdf_fast(path: str):
    """
    Fast extraction:
    1. Find "Tailpipe (EBH02) bench results - not driftcorrected"
    2. Extract Specific Results section
    3. Get NOx+HC (Emission Limits), S_Nox, THC (from Specific Results)
    """
    nox_hc = ""
    s_nox = ""
    thc = ""
    ...
    try:
        with pdfplumber.open(path) as pdf:
            ...
            # Check if this is a Tailpipe report
            if "Tailpipe (EBH02) bench results - not driftcorrected" not in full_text:
                return nox_hc, s_nox, thc, s_thc, co, pm, pn, "Not a Tailpipe report"
            
            lines = full_text.splitlines()
            
            # ===== Find Emission Limits Section =====
            for i, line in enumerate(lines):
                if "Emission limits - Stage V NRE" in line:
                    # Look for NOX+HC in next 15 lines
                    for j in range(i, min(i+15, len(lines))):
                        if "NOX+HC" in lines[j]:
                            nox_hc = extract_value_from_line(lines[j])
                        if "CO uncorrected" in lines[j]:
                            co = extract_value_from_line(lines[j])
                        if "Particulate mass" in lines[j]:
                            pm = extract_value_from_line(lines[j])
                        if "Particulate number" in lines[j]:
                            pn = extract_value_from_line(lines[j])
                    break
            
            # ===== Find Specific Results Section =====
            specific_start = -1
            for i, line in enumerate(lines):
            ...
            if specific_start != -1:
                # Search within Specific Results section (next 20 lines)
                for i in range(specific_start, min(specific_start + 20, len(lines))):
                    line = lines[i]
                    
                    # Extract S_Nox from "NOX [g/kWh]  2.830"
                    if re.search(r'^\s*NOX?\s*\[g/kWh\]', line, re.IGNORECASE):
                        val = extract_value_from_line(line)
                        if val:
                            s_nox = val
                    
                    # Extract THC from "THC [g/kWh]  0.046"
                    if re.search(r'^\s*THC\s*\[g/kWh\]', line, re.IGNORECASE):
                        val = extract_value_from_line(line)
                        if val:
                            thc = val
                            s_thc = val  # S_THC = THC
        ...
        return nox_hc, s_nox, thc, s_thc, co, pm, pn, f"Error: {str(e)}"
    
    return nox_hc, s_nox, thc, s_thc, co, pm, pn, remark
...
class App(tk.Tk):
    ...
    def _build_ui(self):
        ...
        # ===== TITLE =====
        title_label = ttk.Label(main_frame, text="📊 Summary Sheet Tool v16.0", style='Title.TLabel')
    ...
    def _process_file_to_row(self, f):
        ...
        nox_hc = s_nox = thc = s_thc = co = pm = pn = ""
        remark = os.path.basename(f)
        low = f.lower()
        
        if low.endswith(".pdf"):
            nox_hc, s_nox, thc, s_thc, co, pm, pn, _remark = parse_pdf_fast(f)
        ...
        return [date, test_id, test_name, nox_hc, s_nox, thc, s_thc, co, pm, pn, remark] + add_list
